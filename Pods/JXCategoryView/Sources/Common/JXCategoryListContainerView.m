...
@interface JXCategoryListContainerCollectionView : UICollectionView

@end

@implementation JXCategoryListContainerCollectionView
...
@interface JXCategoryListContainerScrollView : UIScrollView

@end

@implementation JXCategoryListContainerScrollView
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if ([gestureRecognizer isMemberOfClass:NSClassFromString(@"UIScrollViewPanGestureRecognizer")]) {
        CGFloat velocityX = [(UIPanGestureRecognizer *)gestureRecognizer velocityInView:gestureRecognizer.view].x;
        //x大于0就是往右滑
...
@property (nonatomic, strong) JXCategoryListContainerCollectionView *collectionView;
...
- (void)initializeViews {
    ...
    if (self.containerType == JXCategoryListContainerType_ScrollView) {
        if (self.delegate &&
            ...
            _scrollView = [[JXCategoryListContainerScrollView alloc] init];
        }
        ...
        if (self.delegate &&
            [self.delegate respondsToSelector:@selector(scrollViewClassInlistContainerView:)] &&
            [[self.delegate scrollViewClassInlistContainerView:self] isKindOfClass:object_getClass([JXCategoryListContainerCollectionView class])]) {
            _collectionView = (JXCategoryListContainerCollectionView *)[[[self.delegate scrollViewClassInlistContainerView:self] alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        }else {
            _collectionView = [[JXCategoryListContainerCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        }
