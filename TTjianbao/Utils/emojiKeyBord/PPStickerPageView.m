...
NSUInteger const PPStickerPageViewMaxEmojiCount = 31;
static NSUInteger const PPStickerPageViewLineCount = 5;
static NSUInteger const PPStickerPageViewButtonPerLine = 7;
static CGFloat const PPStickerPageViewEmojiButtonLength = 32.0;
static CGFloat const PPStickerPageViewEmojiButtonVerticalMargin = 17.0;
static CGFloat const PPStickerPageViewEmojiButtonFirstToo = 15;
...
@property (nonatomic, strong) UIScrollView * emojiScroll;
@end
...
- (id)initWithFrame:(CGRect)frame reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithFrame:frame]) {
        self.emojiScroll = [[UIScrollView alloc] init];
...
//            [button addGestureRecognizer:longPressRecognizer];
            ...
            [self.emojiScroll addSubview:button];
        }
        self.emojiButtons = emojiButtons;
        [self addSubview:self.emojiScroll];
...
- (void)layoutSubviews
{
    ...
    self.emojiScroll.frame = self.bounds;
    self.emojiScroll.contentSize = CGSizeMake(self.width, PPStickerPageViewLineCount * 50 + 50);
    ...
    for (PPButton *button in self.emojiButtons) {
        ...
        CGFloat minY = line * (PPStickerPageViewEmojiButtonLength + PPStickerPageViewEmojiButtonVerticalMargin) + PPStickerPageViewEmojiButtonFirstToo;
...
//    CGFloat minDeleteY = (PPStickerPageViewLineCount - 1) * (PPStickerPageViewEmojiButtonLength + PPStickerPageViewEmojiButtonVerticalMargin);
    if (self.hasReturn) {
        self.deleteButton.frame = CGRectMake(screenWidth - 70 - 56 - 8, self.height - 52, 56, 40);
    }else {
        self.deleteButton.frame = CGRectMake(screenWidth - 70, self.height - 50, 56, 40);
    }
