...
@interface JHMyNewsViewController ()<UITableViewDelegate, UITableViewDataSource, JHDetailCollectionDelegate>
...
@implementation JHMyNewsViewController
...
- (void)loadUserPubTopicData {
    ...
    [HttpRequestTool getWithURL:url Parameters:nil successBlock:^(RequestModel * _Nullable respondObject) {
        ...
        if (self.collectionArray.count > 0) {
            self.contentTableView.tableHeaderView.height = 40;
            [self.contentTableView reloadData];
        }else{
            self.contentTableView.tableHeaderView.height = 0;
            [self.contentTableView reloadData];
        }
...
- (void)loadData:(BOOL)isRefresh {
    ...
    [parameters setValue:self.lastDate forKey:@"lastdate"];
    NSString *url = COMMUNITY_FILE_BASE_STRING(@"/auth/user/content/atme");

    [HttpRequestTool getWithURL:url Parameters:parameters successBlock:^(RequestModel * _Nullable respondObject)
    {
        @strongify(self);
        NSLog(@"@我的动态---%@",respondObject.data);
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.isLoading = NO;
                //self.lastdate = [respondObject.data[@"last_date"] integerValue];
                if (list.count > 0) {
                    NSArray *array = respondObject.data;
                    NSDictionary *dic = [array objectAtIndex:0];
                    self.lastDate = dic[@"last_date"];
