...
- (void)setModel:(JHChatModel *)model
{
    ...
    self.headerImageView.frame = CGRectMake(ScreenW - 15 - HEADER_WIDTH, 10 + top_pad, HEADER_WIDTH, HEADER_WIDTH);
    ...
    if ([model.type integerValue] == 0)
    {
        ...
        self.bgView.backgroundColor = YELLOW_COLOR;
        ...
        self.bgView.frame = CGRectMake(ScreenW - PADDING - HEADER_WIDTH - 10 - introWidth - 20, 10 + top_pad, introWidth + 20, introHeight + 10*2);
        
        self.cornerImageView.frame = CGRectMake(ScreenW - PADDING - HEADER_WIDTH - 10 - introWidth - 20 + introWidth + 20 - 1, 10 + top_pad, 7, 40);
...
//        [self.picImageView sd_setImageWithURL:[NSURL URLWithString:model.image_thumb] placeholderImage:nil completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL)
//        {
//            [MBProgressHUD hideHUDForView:self.picImageView animated:YES];
//        }];
        ...
        if (image_width >= MAX_WIDTH)
        {
            ...
            self.bgView.frame = CGRectMake(ScreenW - PADDING - HEADER_WIDTH - 5 - MAX_WIDTH, 10 + top_pad, MAX_WIDTH,  height);
        }
        else
        {
            self.bgView.frame = CGRectMake(ScreenW - PADDING - HEADER_WIDTH - 5 - image_width, 10 + top_pad, image_width,  image_height);
        }
...
+ (CGFloat)heightWithModel:(JHChatModel *)model
{
    ...
    {
        //文字消息
        CGSize size = [JHMeChatCell getSizeWithString:model.content];
...
- (UIImageView *)cornerImageView
{
    if (!_cornerImageView)
    {
        ...
        _cornerImageView.image = [UIImage imageNamed:@"yellow_corner"];
