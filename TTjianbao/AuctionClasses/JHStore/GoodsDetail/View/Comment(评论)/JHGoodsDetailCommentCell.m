...
@interface JHGoodsDetailCommentCell ()
{
    UIImageView *circle;
    UIView * starView;
    UIView * line;
...
@implementation JHGoodsDetailCommentCell
...
//  self.tagsView.backgroundColor=[UIColor yellowColor];
    ...
    // [selfaddGestureRecognizer:_appraisalAnchorView.panGestureRecognizer];
    ...
    [_detailBtn addTarget:self action:@selector(enterEvaluateReportVC) forControlEvents:UIControlEventTouchUpInside];
...
//进入鉴定详情
- (void)enterEvaluateReportVC {
    JHWebViewController *webView = [[JHWebViewController alloc] init];
    webView.urlString = [NSString stringWithFormat:H5_BASE_STRING(@"/jianhuo/app/report/report.html?orderId=%@"), _audienceCommentMode.orderId];
...
- (void)likeBtnClicked {
    if ([self isLgoin]) {
        BOOL isLaud=self.audienceCommentMode.isLaud;
        NSString *url = [NSString stringWithFormat:FILE_BASE_STRING(@"/orderComment/auth/laud?commentId=%@&status=%@"),
                         _audienceCommentMode.Id, isLaud ? @"0":@"1"];
        ...
        [HttpRequestTool postWithURL:url Parameters:nil requestSerializerType:RequestSerializerTypeHttp successBlock:^(RequestModel *respondObject) {
            ...
            @strongify(self);
            self.audienceCommentMode.isLaud = !isLaud;
            int count = [self.audienceCommentMode.laudTimes intValue];
            if (!isLaud) {
                count = count+1;
                self.audienceCommentMode.laudTimes = [NSString stringWithFormat:@"%d",count];
            }
            else {
                count = count-1;
                self.audienceCommentMode.laudTimes = [NSString stringWithFormat:@"%d",count];
            ...
            [self reloadCell:self.audienceCommentMode];
