//
//  JHGoodsDetailHeaderCycleView.m
//  TTjianbao
//
...
@interface JHGoodsDetailHeaderCycleView () <UIScrollViewDelegate>
...
@implementation JHGoodsDetailHeaderCycleView
...
- (void)setHeadImgList:(NSArray<CGoodsImgInfo *> *)headImgList {
    ...
    for (NSInteger i = 0; i < headImgList.count; i++) {
        ...
        if (imgInfo.type == 1) { //资源类型 [1 图片 , 2 视频]
        ...
        // 如果是视频，添加播放按钮
        if (imgInfo.type == 2) { //资源类型 [1 图片 , 2 视频]
        ...
        [imgView addTapBlock:^(UITapImageView * _Nonnull obj) {
            if (imgInfo.type == 1) {
                @strongify(self);
                [self showPhotoBrowserWithIndex:[_photoViews indexOfObject:obj]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollview {
    ...
    _pageNumLabel.hidden = (imgInfo.type == 2 || _totalImgCount <= 1);
    ...
    if (_videoIndex > -1 && _imgIndex > -1) {
        [self scrollToIsVideo:(imgInfo.type == 2)];
    }
}
