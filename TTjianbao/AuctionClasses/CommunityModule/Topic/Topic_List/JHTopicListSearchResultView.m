...
@interface JHTopicListSearchResultView () <UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation JHTopicListSearchResultView
...
- (void)sendSearchRequest {
    @weakify(self);
    [TopicApiManager request_searchAllTopicList:_curModel keyword:_keywordStr completeBlock:^(CTopicModel * _Nullable respObj, BOOL hasError) {
        @strongify(self);
        [self endRefresh];
        ...
        [self configBlankType:YDBlankTypeNoAllTopicSearchList hasData:_curModel.list.count > 0 hasError:hasError offsetY:0 reloadBlock:^(id sender) {
            NSLog(@"点击刷新按钮");
            //[self refresh];
        }];
    }];
...
- (void)configModel:(CTopicModel *)model {
    ...
    if (_curModel.willLoadMore) {
        [_curModel.list addObjectsFromArray:model.list];
    } else {
        _curModel.list = [NSMutableArray arrayWithArray:model.list];
    }
    
    _curModel.canLoadMore = model.list.count > 0;
}
