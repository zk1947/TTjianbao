...
@interface JHOrderApplyReturnView ()<UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, TZImagePickerControllerDelegate,STPickerSingleDelegate>
{
...
@property (strong, nonatomic) UILabel * partialRefundTip;

@end
@implementation JHOrderApplyReturnView
...
-(void)initScrollview{
    ...
    [self.completeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_voucherView.mas_bottom).offset(40);
        make.height.offset(44);
        make.width.offset(ScreenW);
        make.bottom.equalTo(self.contentScroll).offset(-20.f);
    }];
    
}
- (void)initTipView{
    self.partialRefundTip = [[UILabel alloc]init];
    self.partialRefundTip.backgroundColor = HEXCOLOR(0xFFFAF2);
    self.partialRefundTip.textColor = HEXCOLOR(0xFF6A00);
    self.partialRefundTip.font = JHFont(12);
    self.partialRefundTip.textAlignment = NSTextAlignmentCenter;
    [self addSubview:self.partialRefundTip];
    [self.partialRefundTip mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void)initProductView{
    ...
    [_productView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.partialRefundTip.mas_bottom);
...
-(void)initReasonView{
    ...
    [_reasonView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_introduceView.mas_bottom).offset(10);
        make.left.equalTo(self.contentScroll).offset(0);
        make.right.equalTo(self.contentScroll).offset(0);
...
-(void)initOrderNote{
    ...
    [_noteView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_reasonView.mas_bottom).offset(0);
        make.left.equalTo(self.contentScroll).offset(0);
        make.right.equalTo(self.contentScroll).offset(0);
    ...
    [titleTip mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_noteView).offset(12);
        make.left.equalTo(_noteView).offset(12);
        make.width.equalTo(_noteView);
    }];
    
    [noteTextview mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_noteView).offset(5);
        make.left.equalTo(_noteView).offset(5);
        make.right.equalTo(_noteView).offset(-5);
...
//    UILabel * serviceCashLabel=[[UILabel alloc]init];
//    serviceCashLabel.text=@"服务费";
//    serviceCashLabel.font=[UIFont systemFontOfSize:14];
//    //    title.backgroundColor=[UIColor greenColor];
//    serviceCashLabel.textColor=[CommHelp toUIColorByStr:@"#333333"];
//    serviceCashLabel.numberOfLines = 1;
//    serviceCashLabel.textAlignment = UIControlContentHorizontalAlignmentCenter;
//    serviceCashLabel.lineBreakMode = NSLineBreakByWordWrapping;
//    [_introduceView addSubview:serviceCashLabel];
//
//    [serviceCashLabel mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(_introduceView).offset(10);
//        make.left.equalTo(_introduceView).offset(10);
//    }];
//
//    _serviceCash=[[UILabel alloc]init];
//    _serviceCash.text=@"";
//    _serviceCash.font = [UIFont fontWithName:@"DINCondensed-Bold" size:14.f];
//    //    _allPrice.backgroundColor=[UIColor yellowColor];
//    _serviceCash.textColor=[CommHelp toUIColorByStr:@"#ff4200"];
//    _serviceCash.numberOfLines = 1;
//    [_serviceCash setContentCompressionResistancePriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisVertical];
//    _serviceCash.textAlignment = UIControlContentHorizontalAlignmentCenter;
//    _serviceCash.lineBreakMode = NSLineBreakByWordWrapping;
//    [_introduceView addSubview:_serviceCash];
//
//    [_serviceCash mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.centerY.equalTo(serviceCashLabel);
//        make.left.equalTo(serviceCashLabel.mas_right).offset(5);
//    }];
//
//
//    UILabel * returnCashLabel=[[UILabel alloc]init];
//    returnCashLabel.text=@"退款金额";
//    returnCashLabel.font=[UIFont systemFontOfSize:14];
//    returnCashLabel.textColor=[CommHelp toUIColorByStr:@"#333333"];
//    returnCashLabel.numberOfLines = 1;
//    returnCashLabel.textAlignment = UIControlContentHorizontalAlignmentCenter;
//    returnCashLabel.lineBreakMode = NSLineBreakByWordWrapping;
//    [_introduceView addSubview:returnCashLabel];
//
//    [returnCashLabel mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(serviceCashLabel.mas_bottom).offset(10);
//        make.left.equalTo(_introduceView).offset(10);
//        make.bottom.equalTo(_introduceView).offset(-10);
//    }];
//
//    _returnCash=[[UILabel alloc]init];
//    _returnCash.text=@"12";
//    _returnCash.font = [UIFont fontWithName:@"DINCondensed-Bold" size:14.f];
//    //    _allPrice.backgroundColor=[UIColor yellowColor];
//    _returnCash.textColor=[CommHelp toUIColorByStr:@"#ff4200"];
//    _returnCash.numberOfLines = 1;
//    [_returnCash setContentCompressionResistancePriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisVertical];
//    _returnCash.textAlignment = UIControlContentHorizontalAlignmentCenter;
//    _returnCash.lineBreakMode = NSLineBreakByWordWrapping;
//    [_introduceView addSubview:_returnCash];
//
//    [_returnCash mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.centerY.equalTo(returnCashLabel);
//        make.left.equalTo(returnCashLabel.mas_right).offset(5);
//    }];
    
}
 -(void)initIntroduceSubviews:(NSArray*)arr{
     
     UIView * lastView;
     for (int i=0; i<[arr count]; i++) {
         
         JHOrderReturnMoneyMode * mode=[arr objectAtIndex:i];
         UIView *view=[[UIView alloc]init];
         [view setBackgroundColor:[UIColor whiteColor]];
         [_introduceView addSubview:view];
         ...
         desc.text=mode.cash;
         desc.font=[UIFont fontWithName:kFontBoldDIN size:14.f];
         desc.backgroundColor=[UIColor clearColor];
         desc.textColor=[CommHelp toUIColorByStr:@"#ff4200"];
...
//         }];
         ...
         [view mas_makeConstraints:^(MASConstraintMaker *make) {
             ...
             make.left.right.equalTo(_introduceView);
             if (i==0) {
                 make.top.equalTo(_introduceView).offset(10);
             }
             else{
...
- (void)addPhotos{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"相册" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:3-self.allPhotos.count delegate:self];
...
-(void)commit {
    ...
    if (noteTextview.text.length>0) {
        [dic setObject:noteTextview.text forKey:@"memo"];
    }
...
-(void)setOrderReturnMode:(JHOrderReturnMode *)orderReturnMode{
    _orderReturnMode=orderReturnMode;
    if (_orderReturnMode.partialRefundFlag) {
        self.partialRefundTip.text = [NSString stringWithFormat:@"该订单已完成部分退款，退款金额为￥%@",_orderReturnMode.partialRefundAmount];
        [self.partialRefundTip mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo((self.contentScroll));
            make.left.right.equalTo(self);
     ...
     NSMutableArray * arr=[NSMutableArray array];
    if (_orderReturnMode.appraisalFeeBuyer) {
        JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
        mode.name=@"鉴定费";
        mode.cash=[@"-¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.appraisalFeeBuyer)];
        [arr addObject:mode];
    }
    if (_orderReturnMode.certificateFeeBuyer) {
           JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
           mode.name=@"证书费";
           mode.cash=[@"-¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.certificateFeeBuyer)];
           [arr addObject:mode];
       }
    if (_orderReturnMode.expressFeeBuyer) {
        JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
        mode.name=@"物流费";
        mode.cash=[@"-¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.expressFeeBuyer)];
        [arr addObject:mode];
    }
    
    if (_orderReturnMode.customerBountyReturn) {
        JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
        mode.name=@"津贴";
        mode.cash=[@"¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.customerBountyReturn)];
        [arr addObject:mode];
    }
    
    if (_orderReturnMode.customerCashReturn) {
        JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
        mode.name=@"现金退款";
        mode.cash=[@"¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.customerCashReturn)];
        [arr addObject:mode];
    }
    if (_orderReturnMode.customerCouponReturn) {
        JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
        mode.name=@"代金券";
        mode.cash=[@"¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.customerCouponReturn)];
        [arr addObject:mode];
    }
    if (_orderReturnMode.platformReturn) {
        JHOrderReturnMoneyMode * mode=[[JHOrderReturnMoneyMode alloc]init];
        mode.name=@"红包";
        mode.cash=[@"¥" stringByAppendingString:OBJ_TO_STRING(_orderReturnMode.platformReturn)];
        [arr addObject:mode];
    }
   
      [self initIntroduceSubviews:arr];
