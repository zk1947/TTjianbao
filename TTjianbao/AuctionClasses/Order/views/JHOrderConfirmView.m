...
@property ( nonatomic)   BOOL fromC2C;

@end
@implementation JHOrderConfirmView
...
-(void)configTimeDown:(JHOrderDetailMode*)mode{
    //    JH_WEAK(self)
    //    self.remainTimeView.time=[CommHelp getHMSWithSecond:[CommHelp dateRemaining:mode.payExpireTime]];
    //    if (timer) {
    //        [timer stopGCDTimer];
    //    }
    ...
    //        weakSelf.remainTimeView.time=[CommHelp getHMSWithSecond:presentTime];
    //    } finish:^{
    //        weakSelf.remainTimeView.time = @"订单已取消";
    //    }];
}
-(void)setOrderConfirmMode:( JHOrderDetailMode*)orderConfirmMode{
    ...
    // 刷新title信息
    [self.tipView setSubmitOrdersMode:_orderConfirmMode];
    //商品信息 商品标签
...
//    if (_orderConfirmMode.directDelivery) {
//        _tipView.desc = [[JHBusinessModelTitleConfig businessModelTitleConfig] objectForKey:@"JHStraightHairConfigKey"];
//    }
...
-(void)onClickBtnAction:(UIButton*)sender{
    ...
    self.addressMode = self.addressView.addressMode;
...
-(void)changeGoodsCount:(UIButton*)button{
    ...
    if (button.tag==2) {
        if (!self.orderConfirmMode.isNewuGoods) {
            if (self.orderConfirmMode.showType == 3) { /// 秒杀
                if (self.goodsCount+1>self.orderConfirmMode.seckillMaxNum) {
                    [self makeToast:@"您已超出最大购买数量" duration:1 position:CSToastPositionCenter];
                    return;
                }
            } else {
