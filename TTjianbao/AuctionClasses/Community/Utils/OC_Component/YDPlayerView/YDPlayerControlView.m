...
@interface YDPlayerControlView () <ZFSliderViewDelegate>
...
- (void)makeSubViewsAction {
    ...
    [self.closeBtn addTarget:self action:@selector(closeButtonClickAction:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)layoutSubviews {
    ...
    min_h = (iPhoneX && self.player.isFullScreen) ? 100 : 34;
    min_x = 0;
    ...
    min_w = 62;
    min_h = 28;
    min_y = (self.bottomToolView.zf_height - min_h)/2 + 4;
    ...
    if (self.player.isFullScreen) {
        min_w = 62;
        min_h = 28;
        min_x = self.bottomToolView.zf_width - min_w - ((iPhoneX && self.player.isFullScreen) ? 44: 15);;
        min_y = 0;
...
/// 显示关闭按钮
- (void)showCloseBtnWithAnimated:(BOOL)animated {
    [UIView animateWithDuration:animated ? self.autoFadeTimeInterval : 0 animations:^{
...
/// 隐藏关闭按钮
- (void)hideCloseBtnWithAnimated:(BOOL)animated {
    [UIView animateWithDuration:animated ? self.autoFadeTimeInterval : 0 animations:^{
...
/// 显示展开按钮
- (void)showFullScreenBtnWithAnimated:(BOOL)animated {
    [UIView animateWithDuration:animated ? self.autoFadeTimeInterval : 0 animations:^{
...
/// 隐藏展开按钮
- (void)hideFullScreenBtnWithAnimated:(BOOL)animated {
    [UIView animateWithDuration:animated ? self.autoFadeTimeInterval : 0 animations:^{
 ...
 */
 - (void)showTitle:(nullable NSString *)title coverURLString:(nullable NSString *)coverUrl fullScreenMode:(ZFFullScreenMode)fullScreenMode {
    ...
    [self hideCloseBtnWithAnimated:NO];
...
- (void)videoPlayer:(ZFPlayerController *)videoPlayer orientationWillChange:(ZFOrientationObserver *)observer {
    if (videoPlayer.isSmallFloatViewShow) {
        ...
        if (observer.isFullScreen) {
            [self showCloseBtnWithAnimated:NO];
            [self hideFullScreenBtnWithAnimated:NO];
        } else {
            [self hideCloseBtnWithAnimated:NO];
            [self showFullScreenBtnWithAnimated:NO];
        }
    }
    if (self.controlViewAppeared) {
        [self showControlViewWithAnimated:NO];
...
- (UIButton *)playOrPauseBtn {
    if (!_playOrPauseBtn) {
        ...
        [_playOrPauseBtn setImage:[UIImage imageNamed:@"icon_video_play"] forState:UIControlStateNormal];
        [_playOrPauseBtn setImage:[UIImage imageNamed:@"icon_video_pause"] forState:UIControlStateSelected];
    }
...
- (ZFSliderView *)slider {
    if (!_slider) {
        ...
        _slider.maximumTrackTintColor = kColor666;
        _slider.bufferTrackTintColor  = kColorEEE;
...
- (UIButton *)fullScreenBtn {
    if (!_fullScreenBtn) {
        ...
        [_fullScreenBtn setImage:[UIImage imageNamed:@"icon_video_full_screen"] forState:UIControlStateNormal];
    }
...
- (UIButton *)closeBtn {
    if (!_closeBtn) {
        ...
        [_closeBtn setImage:[UIImage imageNamed:@"icon_video_close"] forState:UIControlStateNormal];
    }
    return _closeBtn;
}
...
- (ZFSliderView *)bottomPgrogress {
    if (!_bottomPgrogress) {
        ...
        _bottomPgrogress.maximumTrackTintColor = kColor666;
        _bottomPgrogress.bufferTrackTintColor  = kColorEEE;
        _bottomPgrogress.minimumTrackTintColor = kColorMain; //[UIColor whiteColor];
