...
+ (instancetype)gradientProgressViewWithFrame:(CGRect)frame
    ...
    progressView.gradientCGColors = @[(id)[UIColor whiteColor].CGColor,
...
- (void)startRendering {
    ...
    if (_animation) {
        if (_showTitle) {
            if(_style == XZMGradientProgressStyleLine)[self.titleLabel.layer addAnimation:[self basicAnimationWithKey:@"position" toValue:[NSValue valueWithCGPoint:CGPointMake(centerX, ProgressH/2.f)]] forKey:@"position"];
        ...
        [self.maskLayer addAnimation:[self basicAnimationWithKey:@"strokeEnd" toValue:@(_progress)] forKey:@"strokeEnd"];
