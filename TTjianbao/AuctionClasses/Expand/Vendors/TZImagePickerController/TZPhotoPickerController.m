...
- (void)showTitleWithString:(NSString *)title
{
    if(title && IS_STRING(title))
    {
        ...
        attach.image = [UIImage tz_imageNamedFromMyBundle:@"tz_photo_title_icon_1"];
        attach.bounds = CGRectMake(0, 3 , 14, 7);
        NSAttributedString *imgStr = [NSAttributedString attributedStringWithAttachment:attach];
        [textAttrStr appendAttributedString:imgStr];
        
        self.jhTitleLabel.attributedText = textAttrStr;
    }
}
...
- (void)viewDidLoad {
    ...
    [self showTitleWithString:_model.name];

    [self initRightButtonWithName:@"下一步" action:@selector(doneButtonClick)];
    [self.jhRightButton setTitleColor:RGB153153153 forState:UIControlStateNormal];
    self.jhRightButton.jh_fontNum(12);
    [self.jhRightButton setBackgroundImage:[UIImage imageWithColor:RGB(238, 238, 238)] forState:UIControlStateNormal];
    
    [self.jhRightButton setBackgroundImage:JHImageNamed(@"publish_button") forState:UIControlStateSelected];
    [self.jhRightButton setTitleColor:RGB515151 forState:UIControlStateSelected];
    
    [self.jhRightButton jh_cornerRadius:13];
    [self.jhRightButton mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.jhNavView).offset(-9);
    }];
    
    self.jhRightButton.hidden = (tzImagePickerVc.maxImagesCount <= 0);

    @weakify(self);
    [self.jhTitleLabel jh_addTapGesture:^{
        @strongify(self);
        [self showAlbum];
    }];
...
{
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    [JHAlbumSelectView showAlbumSelectViewWithView:self.view allowPickingImage:tzImagePickerVc.allowPickingImage allowPickingVideo:tzImagePickerVc.allowPickingVideo title:self.model.name dataBlock:^(TZAlbumModel * _Nonnull model) {
        self->_model = model;
        self->_models = [NSMutableArray arrayWithArray:self->_model.models];
        [self showTitleWithString:model.name];
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        _collectionView.backgroundColor = APP_BACKGROUND_COLOR;
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoLabel.textColor = kColor999;
    ...
    if (tzImagePickerVc.photoPickerPageUIConfigBlock) {
        tzImagePickerVc.photoPickerPageUIConfigBlock(_collectionView, _bottomToolBar, _originalPhotoButton, _originalPhotoLabel, _numberLabel, _divideLine);
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = UI.tabBarAndBottomSafeAreaHeight;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = UI.navBarHeight;
        if (!isStatusBarHidden && isFullScreen) top += UI.statusBarHeight;
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake(ScreenW / 2.f - 35, 0, fullImageWidth + 56, 50);
...
//    _numberLabel.frame = _numberImageView.frame;
    ...
    if (tzImagePickerVc.photoPickerPageDidLayoutSubviewsBlock) {
        tzImagePickerVc.photoPickerPageDidLayoutSubviewsBlock(_collectionView, _bottomToolBar, _originalPhotoButton, _originalPhotoLabel, _numberLabel, _divideLine);
    }
}
...
- (void)doneButtonClick {
    ...
    if(!self.jhRightButton.selected)
    {
        return;
    }
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
...
- (void)refreshBottomToolBarStatus {
    ...
    _numberLabel.hidden = tzImagePickerVc.selectedModels.count <= 0;
    _numberLabel.text = [NSString stringWithFormat:@"%zd",tzImagePickerVc.selectedModels.count];
    self.jhRightButton.selected = tzImagePickerVc.selectedModels.count > 0 || tzImagePickerVc.alwaysEnableDoneBtn;
    _originalPhotoButton.enabled = tzImagePickerVc.selectedModels.count > 0;
    ...
    if (tzImagePickerVc.photoPickerPageDidRefreshStateBlock) {
        tzImagePickerVc.photoPickerPageDidRefreshStateBlock(_collectionView, _bottomToolBar, _originalPhotoButton, _originalPhotoLabel, _numberLabel, _divideLine);;
    }
}
