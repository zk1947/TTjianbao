...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:JHImageNamed(@"navi_icon_back_black") forState:UIControlStateNormal];
    [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(backButtonClick) forControlEvents:UIControlEventTouchUpInside];
    
    _selectButton = [[UIButton alloc] initWithFrame:CGRectZero];
    [_selectButton setImage:tzImagePickerVc.photoOriginDefImage forState:UIControlStateNormal];
    [_selectButton setImage:tzImagePickerVc.photoOriginSelImage forState:UIControlStateSelected];
...
- (void)configBottomToolBar {
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setImage:_tzImagePickerVc.photoOriginDefImage forState:UIControlStateNormal];
    ...
    _doneButton.jh_title(@"下一步").jh_titleColor(UIColor.blackColor).jh_fontNum(12);
    [_doneButton setBackgroundImage:JHImageNamed(@"publish_button") forState:UIControlStateNormal];
    [_doneButton jh_cornerRadius:13];
    ...
    if (_tzImagePickerVc.photoPreviewPageUIConfigBlock) {
        _tzImagePickerVc.photoPreviewPageUIConfigBlock(_collectionView, _naviBar, _backButton, _selectButton, _toolBar, _originalPhotoButton, _originalPhotoLabel, _doneButton);
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake(ScreenW/2.f - 35, 0, fullImageWidth + 56, 44);
        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 42, 0, 80, 44);
    }
    _doneButton.frame = CGRectMake(ScreenW - 71, 9, 56, 26);
    
    [self configCropView];
    
    if (_tzImagePickerVc.photoPreviewPageDidLayoutSubviewsBlock) {
        _tzImagePickerVc.photoPreviewPageDidLayoutSubviewsBlock(_collectionView, _naviBar, _backButton, _selectButton, _toolBar, _originalPhotoButton, _originalPhotoLabel, _doneButton);
    }
}
...
//        NSString *index = [NSString stringWithFormat:@"%d", (int)([_tzImagePickerVc.selectedAssetIds indexOfObject:model.asset.localIdentifier] + 1)];
    ...
    if (_tzImagePickerVc.photoPreviewPageDidRefreshStateBlock) {
        _tzImagePickerVc.photoPreviewPageDidRefreshStateBlock(_collectionView, _naviBar, _backButton, _selectButton, _toolBar, _originalPhotoButton, _originalPhotoLabel, _doneButton);
    }
}
