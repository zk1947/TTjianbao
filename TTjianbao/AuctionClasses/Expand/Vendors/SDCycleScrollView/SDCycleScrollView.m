...
- (void)awakeFromNib
{
    [super awakeFromNib];// fix warning
    [self initialization];
    [self setupMainView];
}
    ...
    switch (self.pageControlStyle) {
        ...
        case SDCycleScrollViewPageContolStyleClassic:
        {
           ...
           // pageControl.pageIndicatorTintColor = self.pageDotColor;
            pageControl.userInteractionEnabled = NO;
            pageControl.currentPage = indexOnPageControl;
           // [pageControl setValue:[UIImage imageNamed:@"圆角矩形-34"] forKeyPath:@"_currentPageImage"];
            //[pageControl setValue:[UIImage imageNamed:@"椭圆-8-副本-2"] forKeyPath:@"_pageImage"];
            [self addSubview:pageControl];
            _pageControl = pageControl;
        }
            break;
            
        case SDCycleScrollViewPageContolStyleJH: {
            JHPageControl *pageControl = [[JHPageControl alloc] init];
            pageControl.hidden = self.imagePathsGroup.count <= 1;
            pageControl.numberOfPages = self.imagePathsGroup.count;
            pageControl.pointSize = 4;
            pageControl.otherMultiple = 1; //其他点w是h的倍数(圆点)
            pageControl.currentMultiple = 3; //选中点的宽度是高度的倍数(设置长条形状)
            pageControl.otherColor = HEXCOLORA(0xFFFFFF, 0.49); //非选中点的颜色
...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated
{
    if (index < _totalItemsCount) {
        [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:animated];
    }
}
...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[JHPageControl class]]) {
        y = self.mainView.sd_height - size.height - 4;
    } else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
           ...
           // [cell.imageView jhSetImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholderImage];
            [cell.imageView setImageWithURL:[NSURL URLWithString:imagePath] placeholder:kDefaultCoverImage options:YYWebImageOptionUseNSURLCache completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
                if (image) {
