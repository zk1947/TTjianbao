...
- (void)viewDidLoad {
    ...
    //     [[NIMAVChatSDK sharedSDK].netCallManager switchVideoEncoder:NIMNetCallVideoCodecHardware];
    [self setUp];
...
-(void)getConnectMicQueue{
    ...
    [HttpRequestTool getWithURL: FILE_BASE_STRING(@"/auth/connectMic/queue")  Parameters:nil successBlock:^(RequestModel *respondObject) {
...
- (void)submitCallId{
    ...
    if (!self.submitCallIdSuccessed && self.submitRetryCount <= 5) {
        ...
        if (self.submitRetryCount > 0) retryTime = 10 * self.submitRetryCount + 5;
...
-(void)submitConnectMicStart:(NSString*)appraiseId{
    
    [HttpRequestTool postWithURL:FILE_BASE_STRING(@"/auth/connectMic/start") Parameters:@{@"appraiseId":appraiseId} requestSerializerType:RequestSerializerTypeHttp successBlock:^(RequestModel *respondObject) {
            ...
            switch (message.messageType) {
                ...
                case NIMMessageTypeCustom:
                {
                    ...
                    if ([attachment isKindOfClass:[JHSystemMsgAttachment class]]) {
                        ...
                        if(type == JHSystemMsgTypeShopwindowAddGoods || type == JHSystemMsgTypeShopwindowRefreash || type == JHSystemMsgTypeShopwindowCount || type == JHSystemMsgTypeShopwindowAudienceCount || type == JHSystemMsgTypeRecycleCountRefresh)
                        {
                            return;
                        }
...
#pragma mark - IM 系统消息回调 NIMSystemNotificationManagerDelegate
             ...
             meeting:(NIMNetCallMeeting *)meeting
{
    ...
    if (!connector) {
        ...
        [HttpRequestTool postWithURL:FILE_BASE_STRING(@"/auth/connectMic/forceClose")  Parameters:parameters requestSerializerType:RequestSerializerTypeHttp  successBlock:^(RequestModel *respondObject) {
        } failureBlock:^(RequestModel *respondObject) {
   ...
   // [self.view makeToast:@"连麦成功" duration:2.0 position:CSToastPositionCenter];
    ...
    if (connector) {
        ...
        //把鉴定id传给后台
        [self submitConnectMicStart:connector.appraiseRecordId];
...
-(void)handleUserLeftMeetingFromUid:(NSString*)uid{
 ...
 //   [self.view makeToast:@"用户已断开连麦" duration:2.0 position:CSToastPositionCenter];
    ...
    if (!self.sendedReporterList[connector.appraiseRecordId]) {
        [JHPublishReportView showWithModel:connector controller:self completeBlock:^(NSDictionary * _Nonnull data, NSString * _Nonnull appraiseRecordId) {
            [self.sendedReporterList setObject:@(1) forKey:appraiseRecordId];
        }];
    }
...
//    _reporter.reportView.mj_y = ScreenH-420-height;
    
}
- (void)keyBoardWillHidden {
//    _reporter.reportView.mj_y = ScreenH-420-UI.bottomSafeAreaHeight;
}
- (void)onShareAction {
...
- (void)dealImage:(UIImage *)image {
    if (self.type) {
        if (self.isCreatAnction) {//创建竞拍
            
            self.isCreatAnction = NO;
            self.innerView.hidden = NO;
            self.auctionListWeb.hidden = NO;
            JHSendOrderView *view = [[JHSendOrderView alloc] initWithFrame:self.view.bounds];
            view.tagArray = self.channel.categories;
            view.hidden = YES;
            view.isAnction = YES;
            view.anchorId = self.channel.anchorId;
            view.roomId = self.chatroom.roomId;
            ...
            [self.sendOrderView.sendOrderSecond showImageViewAction:image];
            self.innerView.hidden = NO;
            self.sendOrderView.sendOrderSecond.hidden = NO;
...
#pragma mark - 连麦队列代理 JHAppraisalListDelegate
//连接
...
- (void)onReporter:(NSInteger)index model:(NTESMicConnector *)model {
    [JHPublishReportView showWithModel:model controller:self completeBlock:^(NSDictionary * _Nonnull data, NSString * _Nonnull appraiseRecordId) {
        [self.sendedReporterList setObject:@(1) forKey:appraiseRecordId];
    }];
}
...
- (void)refuseAppraisal:(NTESMicConnector *) connector{
    ...
    NSString *url= [NSString stringWithFormat:FILE_BASE_STRING(@"/auth/connectMic/anchorCancel?appraiseId=%@"),connector.appraiseRecordId];
...
-(void)forceEndConnectMic:(NSString *)uid{
    ...
    NSDictionary * parameters=@{
                                @"appraiseId":connector.appraiseRecordId,
                                @"timeSpend":timeSpend,
...
-(void)connecterLeft:(NSString *)uid{
    ...
    if (connector) {
        ...
        [HttpRequestTool postWithURL:FILE_BASE_STRING(@"/auth/connectMic/break")  Parameters:parameters requestSerializerType:RequestSerializerTypeHttp  successBlock:^(RequestModel *respondObject) {
...
- (void)showActivity {
    if ([self.homeActivityMode.homeActivityIcon.target.componentName isEqualToString:@"WebDialog"]) {
            ...
            [webview jh_loadWebURL:[urlString stringByAppendingFormat:@"?isSell=%d&isBroad=%d&isAssistant=%d", self.type>0?1:0, 1,(int)self.channel.isAssistant]];
