...
@interface JHAppraisalLinkerView () <UIScrollViewDelegate>
...
@implementation JHAppraisalLinkerView
...
- (void)layoutSubviews {
    ...
    if(!self.detailBtn.selected) {
        self.detailBtn.frame = CGRectMake(left, 0, buttonWidth, buttonHeight);
        self.orderBtn.frame = CGRectMake(left+itemSpace+buttonWidth, 0, buttonWidth, buttonHeight);
        index = 2;
        _sendRedBtn.hidden = YES;
        self.bottomToolBarView.contentSize = CGSizeMake(ScreenW, self.bottomToolBarView.mj_h);
    }
    else {
        if (self.detailBtn.hidden) {
            index = 1;
            _sendRedBtn.hidden = NO;
            self.orderBtn.frame = CGRectMake(left, 0, buttonWidth, buttonHeight);
            self.bottomToolBarView.contentSize = CGSizeMake(ScreenW+ itemSpace+buttonWidth, self.bottomToolBarView.mj_h);
        }
        else {
            self.detailBtn.frame = CGRectMake(left+itemSpace+buttonWidth, 0, buttonWidth, buttonHeight);
            self.orderBtn.frame = CGRectMake(left, 0, buttonWidth, buttonHeight);
            index = 2;
            _sendRedBtn.hidden = NO;
            self.bottomToolBarView.contentSize = CGSizeMake(ScreenW+ itemSpace+buttonWidth, self.bottomToolBarView.mj_h);
    ...
    for (int i = 2; i < self.bottomBtns.count; i++) {
        UIButton *btn = [self.bottomBtns objectAtIndex:i];
        btn.frame = CGRectMake(left+(itemSpace+buttonWidth)*i, 0, buttonWidth, buttonHeight);
    }
...
- (UIButton *)detailBtn {
    if (!_detailBtn) {
        ...
        [_detailBtn setTitle:@"评估报告" forState:UIControlStateSelected];
        [_detailBtn setTitle:@"评估报告" forState:UIControlStateDisabled];
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j<5; j++) {
                ...
                if (index<_dataList.count) {
                    JHLinkerTableViewCell *cell = [[JHLinkerTableViewCell alloc] initWithFrame:CGRectMake(p*ScreenW+space + (space + ww) * j, hh * i, ww, hh)];
...
- (void)detailAction:(UIButton *)btn {
    ...
    if (btn.selected) {
        if (_delegate && [_delegate respondsToSelector:@selector(onReporter:model:)]) {
            [_delegate onReporter:self.index model:self.dataList[self.index]];
        }
...
- (void)refreshData {
    for (NSInteger i = 0; i<self.itemsArray.count; i++) {
        JHLinkerTableViewCell *cell = self.itemsArray[i];
...
-(void)setTimeCount:(NSInteger)timeCount{
    ...
    if (self.statusType==1) {
        ...
        [_linkBtn setTitle:[NSString stringWithFormat:@"连麦中...%ld",(long)timeCount] forState:UIControlStateDisabled];
