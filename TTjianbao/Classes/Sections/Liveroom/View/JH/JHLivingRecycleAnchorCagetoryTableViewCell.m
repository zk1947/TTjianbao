//
//  JHLivingRecycleAnchorCagetoryTableViewCell.m
//  TTjianbao
//
...
@interface JHLivingRecycleAnchorCagetoryCollectionCell : UICollectionViewCell
@property (nonatomic, strong) UILabel *nameLabel;
- (void)setViewModel:(NSString *)str;
@end

@implementation JHLivingRecycleAnchorCagetoryCollectionCell
...
@interface JHLivingRecycleAnchorCagetoryTableViewCell ()<
...
@implementation JHLivingRecycleAnchorCagetoryTableViewCell
...
- (void)showFeeInfoView {
    [self.infoCollectionView registerClass:[JHLivingRecycleAnchorCagetoryCollectionCell class] forCellWithReuseIdentifier:NSStringFromClass([JHLivingRecycleAnchorCagetoryCollectionCell class])];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    JHLivingRecycleAnchorCagetoryCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([JHLivingRecycleAnchorCagetoryCollectionCell class]) forIndexPath:indexPath];
...
- (void)updateData:(NSArray *)dataArr {
    ...
    if ([dataArr count] > 0) {
        ...
        CGFloat height = (self.dataArray.count/4 + self.dataArray.count%4) *25.f;
        [self.infoCollectionView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.contentView).offset(10);
            make.right.mas_equalTo(self.contentView).offset(-10);
            make.top.mas_equalTo(titleLabel.mas_bottom).offset(6.f);
