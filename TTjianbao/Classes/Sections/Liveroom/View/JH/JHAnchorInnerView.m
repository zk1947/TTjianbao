...
@interface JHAnchorInnerView()<NTESLiveActionViewDelegate,NTESTextInputViewDelegate,NTESTimerHolderDelegate,NTESLiveBypassViewDelegate,NTESLiveCoverViewDelegate,NTESLiveChatViewDelegate, JHLiveBottomBarDelegate,UIGestureRecognizerDelegate, JHAutoRunViewDelegate,JHLiveRoomListViewDelegate>{
...
@end
@implementation JHAnchorInnerView
...
- (void)reloadAnctionWeb {
    if (self.type>0) {
        [self.auctionWeb jh_loadWebURL:AuctionURL(self.type>0, self.isAnchor, self.type == 2)];
    }
...
- (void)updateRoleType:(JHLiveRole)roleType{
    ...
    if (roleType == JHLiveRoleAnchor) {
        ...
        _anchorActionView.top = ScreenH-UI.bottomSafeAreaHeight-49- (ButtonHeight)*(ButtonCount - 1);
        _anchorActionView.lightBtn.hidden = YES;
        _anchorActionView.muteListBtn.hidden = NO;
...
- (void)updateSendRedpacketButton:(BOOL)isOn{
    self.anchorActionView.sendRedpacketBtn.hidden=!isOn;
}
...
- (void)layoutSubviews
{
    ...
    _bottomBar.frame = CGRectMake(0,ScreenH-UI.bottomSafeAreaHeight-49 ,ScreenW,49);
...
- (JHDotButtonView *)stoneOrderBtn {
    ...
    if (!_stoneOrderBtn) {
        ...
        _stoneOrderBtn.frame = CGRectMake(ScreenW-80, self.saleAnchorActionView.top-33, 80, 33);
...
- (  JHSaleAnchorActionView *)saleAnchorActionView {
    if (!_saleAnchorActionView) {
        ...
        _saleAnchorActionView.hidden = YES;
        [_saleAnchorActionView resetActionView:1 andShowSendLivingTip:(self.channel.canSendLivingTip == 1)&&[self.channel.channelCategory isEqualToString:JHRoomTypeNameRestoreStone]];//
...
- (JHLiveBottomBar *)bottomBar {
    if (!_bottomBar) {
        ...
        _bottomBar.hasShanGou = [self.channel.canFlashPurchase isEqualToString:@"1"];
...
-(JHReplyMessageView *)replyMessageView
{
    if(!_replyMessageView)
    {
        ...
        _replyMessageView.roomRole = @(JHRoomRoleAnchor);
        _replyMessageView.frame = CGRectMake(0, 0, 154, 192);
        _replyMessageView.hidden = YES;
//        _replyMessageView.frame = CGRectMake(0, ScreenH, ScreenW, 44);
...
- (void)clickListBtnAction:(UIButton *)btn Trailing:(CGFloat)trailing
{
    ...
    //福袋包含在 卖场直播间、定制直播间、回收直播间、原石直播间（除原石回血、鉴定）
    ...
    NSArray *arr1;
      if(self.canAuction)
      {
          arr1 = @[@{@"title":@"发红包",@"icon":@"live_room_more_redpacket_icon"},@{@"title":@"竞拍",@"icon":@"live_room_more_auction_icon"},@{@"title":@"订单",@"icon":@"live_room_more_order_icon"},@{@"title":@"联系平台",@"icon":@"live_room_more_service_icon"}];
      }else
      {
          arr1 = @[@{@"title":@"发红包",@"icon":@"live_room_more_redpacket_icon"},@{@"title":@"订单",@"icon":@"live_room_more_order_icon"},@{@"title":@"联系平台",@"icon":@"live_room_more_service_icon"}];
      }
    [arr addObjectsFromArray:arr1];
    CGFloat height = arr.count*34+6;
    CGFloat width = 84.0;
    CGFloat x = kScreenWidth - width - trailing+37/2.0 + 5.5f;
