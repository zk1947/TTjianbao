...
@interface JHChatCustomTipCell ()
@property (nonatomic, strong) UILabel *titleLabel;
@end
@implementation JHChatCustomTipCell
...
- (void)setupData {
    @weakify(self)
    [[RACObserve(self.message, customTipInfo)
      takeUntil:self.rac_prepareForReuseSignal]
     subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        JHChatCustomTipInfo *info = x;
        
        if (self.message.senderType == JHMessageSenderTypeMe) {
            self.titleLabel.text = info.senderTip;
        }else {
            self.titleLabel.text = info.receiverTip;
        }
