...
@implementation JHZeroTableView
...
@interface JHZeroAuctionViewController ()<UITableViewDelegate,UITableViewDataSource,JHZeroAuctionListCellDelegate>

@property (nonatomic, strong) JHZeroAuctionHeadView *headView;

@property (nonatomic, strong) JHZeroAuctionListCell *lastCell;
...
@implementation JHZeroAuctionViewController
...
- (void)setupView{
    ...
    self.headView.headChooseBlock = ^(JHZeroAuctionRequestModel * _Nonnull model) {
        @strongify(self);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    self.jhTitleLabel.text = bottomLineHidden ? @"" : self.headView.headModel.zeroAuctionConfBo.title;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    JHZeroAuctionListCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([JHZeroAuctionListCell class])];
    cell.delegate = self;
    @weakify(self);
    cell.haveDataBlock = ^(BOOL noData, JHZeroAuctionModel * _Nonnull listModel) {
        @strongify(self);
        self.noData = noData;
...
//                [JHNewStoreHomeReport jhNewStoreHomeBoutiqueClickReport:boutiqueName zc_type:2 zc_id:showId store_from:@"商品推荐列表"];
    ...
    self.lastCell = cell;
    if (!cell) {
        cell = [[JHZeroAuctionListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([JHZeroAuctionListCell class])];
    }
    return cell;
}
...
- (JHZeroTableView *)tableView {
    if (!_tableView) {
        _tableView = [[JHZeroTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
        ...
        [_tableView registerClass:[JHZeroAuctionListCell class] forCellReuseIdentifier:NSStringFromClass([JHZeroAuctionListCell class])];
    }
    return _tableView;
}

- (JHZeroAuctionHeadView *)headView{
    if (!_headView) {
        JHZeroAuctionHeadView *view = [[JHZeroAuctionHeadView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, KHeadH)];
        view.vc = self;
        _headView = view;
    }
    return _headView;
