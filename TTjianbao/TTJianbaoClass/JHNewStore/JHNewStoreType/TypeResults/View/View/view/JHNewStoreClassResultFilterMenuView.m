//
//  JHNewStoreClassResultFilterMenuView.m
//  TTjianbao
//
...
@interface JHNewStoreClassResultFilterMenuView ()<UITextFieldDelegate>
...
@property (nonatomic, assign) BOOL auctionStatus;
...
@implementation JHNewStoreClassResultFilterMenuView
...
- (void)setTitleTagIndex:(NSInteger)titleTagIndex{
    _titleTagIndex = titleTagIndex;
    [self initSubviews];
}
...
- (void)setupServiceView{
    ...
    for (int i = 0; i < serviceBtnArray.count; i++) {
        ...
        [btn setTitle:serviceBtnArray[i] forState:UIControlStateNormal];
        ...
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.serviceView).offset(18 + 100*i);
        }];
    }
}
- (void)setupAuctionView{
    //拍卖
    [self.bgView addSubview:self.auctionView];
    [self.auctionView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.bgView);
        make.height.mas_equalTo(60);
        make.top.equalTo(self.serviceView.mas_bottom).offset(20);
    }];
    UILabel *titleLabel = [UILabel jh_labelWithText:@"拍卖" font:13 textColor:HEXCOLOR(0x333333) textAlignment:NSTextAlignmentLeft addToSuperView:self.auctionView];
    titleLabel.font = JHBoldFont(13);
    [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.auctionView).offset(18);
        make.top.equalTo(self.auctionView);
    }];
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    [btn setTitle:@"即将截拍" forState:UIControlStateNormal];
    [btn setTitleColor:HEXCOLOR(0x333333) forState:UIControlStateNormal];
    [btn setBackgroundImage:[UIImage imageWithColor:HEXCOLOR(0xF7F7F8)] forState:UIControlStateNormal];
    [btn setBackgroundImage:[UIImage imageWithColor:HEXCOLOR(0xFFD70F)] forState:UIControlStateSelected];
    [btn jh_cornerRadius:15];
    ...
    btn.tag = 102;
    [btn addTarget:self action:@selector(clickServiceBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.auctionView addSubview:btn];
    [btn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(titleLabel.mas_bottom).offset(12);
        make.size.mas_equalTo(CGSizeMake(90, 30));
        make.left.equalTo(self.serviceView).offset(18);
...
- (void)setupPriceView{
    ...
    [self.priceView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (self.titleTagIndex == 2) {
            ...
            make.top.equalTo(self.auctionView.mas_bottom).offset(20);
        }
    }];
...
- (void)clickServiceBtnAction:(UIButton *)sender{
    ...
    if (index < 2) {
        ...
        if (index == 0) {//商家直发
            self.serviceIndex = sender.selected ? 2 : 0;
        } else {//平台鉴定
    ...
    //拍卖按钮
        ...
        sender.selected = !sender.selected;
        self.auctionStatus = sender.selected ? YES : NO;
    }
...
- (void)clickBottomBtnAction:(UIButton *)sender{
    ...
    //重置
    if (index == 0) {
        if (self.serviceIndex > 0 || self.auctionStatus || self.lowTextField.text.length > 0 || self.highTextField.text.length > 0) {
            [self clearHistory];
            if (self.delegate && [self.delegate respondsToSelector:@selector(filterViewSelectedOfService:auction:lowPrice:highPrice:isFilteredInfo:)]) {
                [self.delegate filterViewSelectedOfService:self.serviceIndex auction:self.auctionStatus lowPrice:self.lowTextField.text highPrice:self.highTextField.text isFilteredInfo:self.isFilteredInfo];
            }
        }
    }
    //确定
...
///清除历史
- (void)clearHistory{
    int count = self.titleTagIndex==2 ? 2 : 3;
    for (int i = 0; i < count; i++) {
        UIButton *selectBtn = [self viewWithTag:100+i];
        selectBtn.selected = NO;
    }
    self.serviceIndex = 0;
    self.auctionStatus = NO;
