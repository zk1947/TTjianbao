...
@interface JHStealTowerHeadView ()<JHC2CSortMenuViewDelegate,JHC2CClassMenuViewDelegate,JHC2CSubClassTitleScrollViewDelegate,UITextFieldDelegate>
...
@property (nonatomic, strong) JHStealTowerRequestModel *requestModel;
...
@implementation JHStealTowerHeadView
...
- (void)setHeadModel:(JHStealTowerModel *)headModel{
    _headModel = headModel;
    //刷新头部
    if (!_onlyOnce) {
        NSString *imageStr = [_headModel.stealingTowerConfigResponse.coverImg stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        [self.bgImgv jh_setImageWithUrl:imageStr placeHolder:nil];
        self.titLab.text = _headModel.stealingTowerConfigResponse.title;
        self.tagLab.text = _headModel.stealingTowerConfigResponse.label;
        self.classMenuView.subCateIds = _headModel.backFirstCateIds;
        _onlyOnce = YES;
    }
}
...
- (void)setupUI{
    ...
    self.titLab.frame = CGRectMake(0, UI.navBarHeight+30, kScreenWidth, 43);
    [self addSubview:self.titLab];
    
    self.tagLab.frame = CGRectMake(0, self.titLab.bottom + 15, kScreenWidth, 16);
    [self addSubview:self.tagLab];
    
    [self textGradientview:self.titLab bgVIew:self gradientColors:@[(__bridge id)HEXCOLOR(0xF9F1D6).CGColor, (__bridge id)HEXCOLOR(0xFBC58F).CGColor] gradientStartPoint:CGPointMake(0, 0) endPoint:CGPointMake(1, 1)];
    
    [self textGradientview:self.tagLab bgVIew:self gradientColors:@[(__bridge id)HEXCOLOR(0xF9F1D6).CGColor, (__bridge id)HEXCOLOR(0xFBC58F).CGColor] gradientStartPoint:CGPointMake(0, 0) endPoint:CGPointMake(1, 1)];
...
- (UILabel *)tagLab{
    if (!_tagLab) {
        UILabel *lab = [UILabel new];
        lab.text = @"全场1折封封顶 | 大师传承工艺";
        lab.font = JHMediumFont(16);
        lab.textAlignment = NSTextAlignmentCenter;
        lab.adjustsFontSizeToFitWidth = YES;
        _tagLab = lab;
    }
...
- (JHStealTowerRequestModel *)requestModel{
    if (!_requestModel) {
        _requestModel = [JHStealTowerRequestModel new];
