//
//  JHStealTowerListViewController.m
//  TTjianbao
//
...
@interface JHStealTowerListViewController ()<UIScrollViewDelegate,UICollectionViewDelegate, UICollectionViewDataSource,JHShopHotSellConllectionViewLayoutDelegate>
...
@implementation JHStealTowerListViewController
...
- (void)loadData:(JHStealTowerRequestModel *)requestModel completion:(void (^ __nullable)(BOOL finished))completion{
    ...
    @weakify(self);
    [JHStealTowerBusiness loadStealTowerListData:param completion:^(NSError * _Nullable error, JHStealTowerModel * _Nonnull model) {
        @strongify(self);
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        if (completion) {
            completion(YES);
        }
        if (!model.closingProductResponse.resultList || model.closingProductResponse.resultList.count == 0) {
        ...
        [self.hotSellDataArray removeAllObjects];
        [self.hotSellDataArray addObjectsFromArray:model.closingProductResponse.resultList];
