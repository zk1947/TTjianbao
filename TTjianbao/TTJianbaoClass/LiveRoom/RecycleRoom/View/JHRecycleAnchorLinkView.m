...
@interface JHRecycleAnchorLinkView () <UIScrollViewDelegate>
...
@implementation JHRecycleAnchorLinkView
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j<5; j++) {
                ...
                if (index<_dataList.count) {
                    JHRecycleAnchorLinkerCell *cell = [[JHRecycleAnchorLinkerCell alloc] initWithFrame:CGRectMake(p*ScreenW+space + (space + ww) * j, hh * i, ww, hh)];
...
- (void)refreshData {
    for (NSInteger i = 0; i<self.itemsArray.count; i++) {
        JHRecycleAnchorLinkerCell *cell = self.itemsArray[i];
