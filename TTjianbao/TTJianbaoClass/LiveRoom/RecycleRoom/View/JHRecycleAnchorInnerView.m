//
//  JHRecycleAnchorInnerView.m
//  TTjianbao
//
//  Created by jiangchao on 2021/3/29.
//  Copyright © 2021 YiJian Tech. All rights reserved.
//
#import "JHRecycleAnchorInnerView.h"
...
@property (nonatomic, strong) JHRightArrowBtn *waitBuyBtn;//待回收出价
@property (nonatomic, strong) JHRightArrowBtn *waitConfirmBtn; //回收待付款
...
@end
@implementation JHRecycleAnchorInnerView
...
- (void)refreshFirstTopBtn{
    if (self.channel.recycleWaitOfferCount>0) {
        self.waitBuyBtn.hidden = NO;
        [self.waitBuyBtn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(86);
        }];
        [self.waitBuyBtn setTitle:[NSString stringWithFormat:@"回收待出价 %ld",self.channel.recycleWaitOfferCount] forState:UIControlStateNormal];
    }else{
        self.waitBuyBtn.hidden = YES;
        [self.waitBuyBtn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(0);
        }];
    }
    if (self.channel.recycleWaitPayCount > 0) {
        self.waitConfirmBtn.hidden = NO;
        [self.waitConfirmBtn setTitle:[NSString stringWithFormat:@"回收待付款 %ld",self.channel.recycleWaitPayCount] forState:UIControlStateNormal];
    }else{
        self.waitConfirmBtn.hidden = YES;
    }
}
- (void)refreshTopBtnStatus:(NSDictionary *)dic{
    NSString * OfferCount = [NSString stringWithFormat:@"%@",dic[@"recycleWaitOfferCount"]];
    NSString * PayCount = [NSString stringWithFormat:@"%@",dic[@"recycleWaitPayCount"]];
    if ([OfferCount integerValue]>0) {
        self.waitBuyBtn.hidden = NO;
        [self.waitBuyBtn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(86);
        }];
        [self.waitBuyBtn setTitle:[NSString stringWithFormat:@"回收待出价 %@",OfferCount] forState:UIControlStateNormal];
    }else{
        self.waitBuyBtn.hidden = YES;
        [self.waitBuyBtn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(0);
    ...
    if ([PayCount integerValue]>0) {
        self.waitConfirmBtn.hidden = NO;
        [self.waitConfirmBtn setTitle:[NSString stringWithFormat:@"回收待付款 %@",PayCount] forState:UIControlStateNormal];
    }else{
        self.waitConfirmBtn.hidden = YES;
    }
...
- (void)setup
{
    ...
    [self addSubview:self.waitBuyBtn];
    [self addSubview:self.waitConfirmBtn];
    ...
    [self.waitBuyBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(18);
        make.width.mas_equalTo(86);
        make.left.mas_equalTo(self.jadeiteListBtn.mas_right).offset(10);
        make.centerY.mas_equalTo(self.jadeiteListBtn.mas_centerY).offset(0);
    }];
    [self.waitConfirmBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(18);
        make.width.mas_equalTo(86);
        make.left.mas_equalTo(self.waitBuyBtn.mas_right).offset(10);
        make.centerY.mas_equalTo(self.jadeiteListBtn.mas_centerY).offset(0);
    }];
    ...
    [JHLiveRoomPKCategoryModel requestPKCategory:self.channel.channelId successBlock:^(NSString * _Nonnull category,NSString * _Nonnull rankName) {
        if (category.length>0) {
            ...
            [self.jadeiteListBtn mas_updateConstraints:^(MASConstraintMaker *make) {
                make.width.mas_equalTo(0);
            }];
        }
    }];
...
- (void)setJadeiteListStr:(NSString *)jadeiteListStr {
    _jadeiteListStr = jadeiteListStr;
    if (_jadeiteListStr.length <= 0) return;
...
- (JHRightArrowBtn *)waitBuyBtn {
    if (!_waitBuyBtn) {
        _waitBuyBtn = [[JHRightArrowBtn alloc] init];
        [_waitBuyBtn setTitle:@"回收待出价 0" forState:UIControlStateNormal];
        [_waitBuyBtn resetRightArrowStyle:JHRightArrowStyleLight];
        _waitBuyBtn.hidden = YES;
        [_waitBuyBtn addTarget:self action:@selector(waitBuyBtnAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _waitBuyBtn;
}
- (JHRightArrowBtn *)waitConfirmBtn {
    if (!_waitConfirmBtn) {
        _waitConfirmBtn = [[JHRightArrowBtn alloc] init];
        [_waitConfirmBtn setTitle:@"回收待付款 0" forState:UIControlStateNormal];
        [_waitConfirmBtn resetRightArrowStyle:JHRightArrowStyleLight];
        _waitConfirmBtn.hidden = YES;
        [_waitConfirmBtn addTarget:self action:@selector(waitConfirmBtnAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _waitConfirmBtn;
}
- (void)waitBuyBtnAction{
    JHRecycleSquareHomeViewController *vc = [[JHRecycleSquareHomeViewController alloc] init];
    [[JHRootController currentViewController].navigationController pushViewController:vc animated:YES];
}
- (void)waitConfirmBtnAction{
    JHWebViewController *vc = [[JHWebViewController alloc] init];
    vc.urlString = [H5_BASE_STRING(@"/jianhuo/app/recycle/order/orderList.html?") stringByAppendingString:[NSString stringWithFormat:@"index=%@&bussId=%@",@"0", @"1"]];
...
- (JHRightArrowBtn *)jadeiteListBtn{
    if (!_jadeiteListBtn) {
        ...
        [_jadeiteListBtn setTitle:[NSString stringWithFormat:@"    %@",self.jadeiteListStr] forState:UIControlStateNormal];
...
- (void)jadeiteListBtnAction{
    JHLiveRoomPKView * pkview = [[JHLiveRoomPKView alloc] initWithFrame:CGRectZero andType:2 andchannelId:self.channel.channelId];
