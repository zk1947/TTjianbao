//
//  JHRecycleAnchorLiveController.m
//  TTjianbao
//
...
#import "JHRecycleAnchorLinkView.h"
...
@interface JHRecycleAnchorLiveController ()<NIMChatroomManagerDelegate,NTESLiveInnerViewDelegate,NTESLiveAnchorHandlerDelegate,
...
@property (nonatomic, strong) JHRecycleAnchorInnerView *innerView;
...
@property (nonatomic, strong)JHRecycleAnchorLinkView *appraisalList;
...
@implementation JHRecycleAnchorLiveController
...
-(void)getConnectMicQueue{
    ...
    [HttpRequestTool getWithURL: FILE_BASE_STRING(@"/recycle/connectMic/queue")  Parameters:nil successBlock:^(RequestModel *respondObject) {
...
-(void)submitConnectMicStart:(NSString*)applyId{
    
    [HttpRequestTool postWithURL:FILE_BASE_STRING(@"/recycle/connectMic/start") Parameters:@{@"applyId":applyId} requestSerializerType:RequestSerializerTypeHttp successBlock:^(RequestModel *respondObject) {
...
-(void)submitReverseConnectMicStart:(NSString*)applyId{
    
    [HttpRequestTool postWithURL:FILE_BASE_STRING(@"/recycle/reverse/connectMic/start") Parameters:@{@"applyId":applyId} requestSerializerType:RequestSerializerTypeHttp successBlock:^(RequestModel *respondObject) {
            ...
            case NIMMessageTypeCustom:
            {
                ...
                if ([attachment isKindOfClass:[JHSystemMsgAttachment class]]) {
                   ...
                   //移除定制中的订单
                    ...
                    {
                        return;
                    }else if(type == JHSystemMsgTypeRecycleCountRefresh){
                        //刷新顶部数据
                    }
...
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"NotificationNameUpdateOrderNumber" object:nil];
        ...
        case JHSystemMsgTypeForbidLive: {
            ...
            break;
        case NTESLiveCustomNotificationTypeRecycleCountRefresh:{
            //刷新顶部数据
            NSDictionary *dic = dict[@"body"];
            [self.innerView refreshTopBtnStatus:dic];
        }
            break;
        default:
            break;
   ...
   // [self.view makeToast:@"连麦成功" duration:2.0 position:CSToastPositionCenter];
    ...
    if (connector) {
        ...
        if (connector.connectType == 1) {
            [self submitReverseConnectMicStart:connector.applyId];
        }else{
            [self submitConnectMicStart:connector.applyId];
        }
...
- (void)onCanAppraiseAction:(UIButton *)btn {
    [HttpRequestTool putWithURL:FILE_BASE_STRING(@"/channel/canRecycle/auth") Parameters:@{@"yesOrNo":@(btn.selected)} requestSerializerType:RequestSerializerTypeHttp successBlock:^(RequestModel *respondObject) {
...
- (void)doExitLive
{
    ...
    [JHTracking trackEvent:@"endLive" property:@{@"live_duration":@(dur),@"channel_id":self.channel.channelId,@"channel_name":self.channel.title,@"channel_local_id":self.channel.channelLocalId,@"first_channel":@[self.channel.channelCategory]}];
    [self.capture stopLiveStream];
    [self.capture stopVideoCapture];
    [[NTESLiveManager sharedInstance] removeAllConnectors];
...
- (JHRecycleAnchorLinkView *)appraisalList {
    if (!_appraisalList) {
        _appraisalList = [[JHRecycleAnchorLinkView alloc] initWithFrame:CGRectMake(0.f, ScreenH, ScreenW, 265./375.*ScreenW+UI.bottomSafeAreaHeight)];
...
{
    if (!_innerView) {
        _innerView = [[JHRecycleAnchorInnerView alloc] initWithChannel:self.channel frame:self.view.bounds isAnchor:YES];
...
- (void)fetchAnchorInfo {
    ...
    [HttpRequestTool getWithURL:FILE_BASE_STRING(@"/authoptional/appraise") Parameters:@{@"customerId" : self.channel.anchorId} successBlock:^(RequestModel *respondObject) {
        ...
        [self.roomDetailView setLiveRoomAnchorInfoModel:model roleType:JHAudienceUserRoleTypeRecycle];
...
- (void)didPressAnchorAvatar:(NIMChatroomMember *)member {
    if (self.anchorLiveType == JHAnchorLiveAppraiseType) {
        ...
        [JHGrowingIO trackEventId:JHEventAssayerprofile variables:@{JHKeyPagefrom:JHPageFromAppraiseRoom}];
             ...
             self.anchorLiveType == JHAnchorLiveRecycleAssistantType) {///跳转回收师个人主页
        [JHRootController toNativeVC:NSStringFromClass([JHRecycleInfoViewController class])
...
- (void)refuseAppraisal:(NTESMicConnector *) connector{
    ...
    NSString *url= [NSString stringWithFormat:FILE_BASE_STRING(@"/recycle/connectMic/anchorCancel?applyId=%@"),connector.applyId];
...
- (void)anchorCancelLink:(NTESMicConnector *) connector{
    ...
    NSString *url= [NSString stringWithFormat:FILE_BASE_STRING(@"/recycle/reverse/connectMic/anchorCancel?applyId=%@"),connector.applyId];
...
-(void)connecterLeft:(NSString *)uid{
    ...
    if (connector) {
        NSString * urlStr = @"/recycle/connectMic/break";
        if (connector.connectType == 1) {
            urlStr = @"/recycle/connectMic/reverse/break";
        }
        NSString *  timeSpend=[NSString stringWithFormat:@"%ld",([[CommHelp getNowTimeTimestamp] integerValue]-[startTime integerValue])/1000];
...
- (void)onShowInfoWithModel:(NTESMessageModel *)model{
    if (self.anchorLiveType == JHAnchorLiveRecycleType) {
...
- (void)makeCardViewWithModel:(NTESMessageModel *)model {
    ...
    _sendOrderView.orderAction = ^(NTESMessageModel *object, OrderTypeModel *sender) {
        if ([sender.Id isEqualToString:@"normal"]) {//常规订
            ...
            self.normalOrderView.isCustomizeSelf = NO;
            ...
            if (connector) { //用户已经申请连麦  走正向连麦
                ...
                JH_WEAK(self)
                [JHLiveApiManager reverseApplyConnectRecycle:self.channel.roomId viewerId:object.customerId Completion:^(RequestModel *respondObject, NSError *error) {
                    JH_STRONG(self)
                    if (!error) {
                        
                        NTESMicConnector *connector = [[NTESMicConnector alloc] init];
                        connector.uid    = respondObject.data[@"wyAccid"];
                        connector.applyId = respondObject.data[@"applyId"];
    //                    connector.Id    = Id;
                        connector.state  = NTESLiveMicStateWait;
