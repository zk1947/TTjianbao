//
//  JHMallCategoryCollectionCell.m
//  TTjianbao
//
...
@interface JHMallCategoryCollectionCell ()
/** 角标图片*/
...
@implementation JHMallCategoryCollectionCell
...
//    JHSkinModel *model = [JHSkinManager kimFont];
//    if (model.isChange)
//    {
//        if ([model.type intValue] == 1)
//        {
//            label.textColor = COLOR_CHANGE(model.name);
//        }
//    }
...
- (void)bindData {
    ...
    @weakify(self)
    [[RACObserve(manager, sceneVajraTitleModel)
      takeUntil:self.rac_prepareForReuseSignal]
     subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        JHSkinSceneModel *scene = x;
        if (scene == nil) return;
        self.cateTitleLabel.textColor = scene.colorNor;
    }];
}
@end
