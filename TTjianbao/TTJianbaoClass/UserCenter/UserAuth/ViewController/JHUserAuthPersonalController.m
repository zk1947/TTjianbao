//
//  JHUserAuthPersonalController.m
//  TTjianbao
//
...
@interface JHUserAuthPersonalController () <UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation JHUserAuthPersonalController

- (instancetype)init {
    self = [super init];
    if (self) {
        self.authModel = [[JHUserAuthModel alloc] init];
        self.authModel.authType = JHUserAuthTypePersonal;
        self.authModel.authState = [UserInfoRequestManager sharedInstance].user.authState;
    }
    return self;
}
...
- (void)setAuthModel:(JHUserAuthModel *)authModel {
    ...
    if ([_authModel.idCardBackImg isNotBlank]) {
        self.idCardBackImg = _authModel.idCardFrontImg;
    }
}
...
- (void)configUI {
    ...
    _tableView.tableFooterView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenW, 40)];;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {///审核状态的提示信息
        if ([self hasAuth] && self.authModel.authState != JHUserAuthStateUnCommit) {
    ...
    if (indexPath.section == 1) { ///展示身份证信息
