//
//  JHC2CProductInnerChatMainCell.m
//  TTjianbao
//
...
@interface JHC2CProductInnerChatMainCell ()<UIGestureRecognizerDelegate>
...
@implementation JHC2CProductInnerChatMainCell
...
//    }];
    ...
    _timeLabel.text = [NSString stringWithFormat:@"%@", _mainComment.time ? : @"暂无"];
    ///图片评论布局
    [self makeCommentLayouts];
    
    _likeButton.selected = _mainComment.is_like;
    NSString * likeNum = _mainComment.like_num > 0 ? @(_mainComment.like_num).stringValue : @"";
...
- (void)updateLikeButtonStatus:(JHCommentModel *)comment {
    ...
    NSString *likeNum = comment.like_num > 0 ? @(comment.like_num).stringValue : @"";
    [_likeButton setTitle:likeNum forState:UIControlStateNormal];
    if (comment.is_like) {
...
- (void)setupViews {
    ...
    [_likeButton setImage:[UIImage imageNamed:@"c2c_pd_zan_unsel"] forState:UIControlStateNormal];
    [_likeButton setImage:[UIImage imageNamed:@"c2c_pd_zan_sel"] forState:UIControlStateSelected];
    [_likeButton setTitleColor:HEXCOLOR(0x999999) forState:UIControlStateNormal];
    ...
    _likeButton.titleLabel.font = JHFont(11);
    _likeButton.titleEdgeInsets = UIEdgeInsetsMake(0, 4, 0, -4);
