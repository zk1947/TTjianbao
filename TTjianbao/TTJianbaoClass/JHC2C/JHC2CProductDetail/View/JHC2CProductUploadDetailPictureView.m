//
//  JHC2CProductUploadDetailPictureView.m
//  TTjianbao
//
...
@interface JHC2CProductUploadDetailPictureView()
...
//icon_cover_close
@implementation JHC2CProductUploadDetailPictureView
    ...
    for (int i = 0; i<self.imageViewArr.count; i++) {
        ...
        [imageView mas_remakeConstraints:^(MASConstraintMaker *make) {
            if (isNewLine) {
                ...
                make.top.equalTo((lastView ? lastView : self.titleLbl).mas_bottom).offset(14);
            }else{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        for (int i = 0; i< modelArr.count; i++) {
            ...
            imageView.tapImageBlock = ^(JHRecyclePhotoSeletedView * _Nonnull photoView) {
                ...
                if (photoView.model.mediaType == PHAssetMediaTypeImage) {
                    model.image = photoView.model.originalImage;
                }else{
                    model.image = photoView.model.originalImage;
//                    model.mediaUrl = photoView.model.aliossUrl;
                    model.mediaUrl = photoView.model.videoLocalUrl;
                }
...
- (void)addNetProductDetailPictureWithModelArr:(NSArray *)modelArr{
    dispatch_async(dispatch_get_main_queue(), ^{
        for (int i = 0; i< modelArr.count; i++) {
            ...
            //添加图片预览
            imageView.tapImageBlock = ^(JHRecyclePhotoSeletedView * _Nonnull photoView) {
                JHBrowserModel *bModel = [[JHBrowserModel alloc] init];
                if (photoView.editModel.type == 0) {
                    bModel.thumbImageUrl = photoView.editModel.url;
                }else{
                    bModel.thumbImageUrl = photoView.editModel.detailVideoCoverUrl;
                    bModel.mediaUrl = photoView.editModel.url;
                }
                [JHBrowserViewController showBrowser:@[bModel] currentIndex:0 from:[self getCurrentVC]];
            };
