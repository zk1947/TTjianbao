//
//  JHCustomizeOrderSubListViewController.m
//  TTjianbao
//
...
#import "JHBuyerCustomizeOrderListCell.h"
...
@interface JHCustomizeOrderSubListViewController ()<UITableViewDelegate,UITableViewDataSource,JHOrderSegmentViewViewDelegate,JHOrderViewDelegate,UIDocumentInteractionControllerDelegate,UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
{
    NSInteger PageNum;
...
@implementation JHCustomizeOrderSubListViewController
- (void)viewDidLoad {
...
-(void)requestInfo{
    ...
    if (self.isSeller) {
        ...
        url = FILE_BASE_STRING(@"/orderCustomize/auth/orderListBySeller");
    }
    else{
        url = FILE_BASE_STRING(@"/orderCustomize/auth/orderListByBuyer");
    }
    
    [HttpRequestTool postWithURL:url Parameters:@{@"status":self.status,@"pageIndex":[NSString stringWithFormat:@"%ld",(long)PageNum],@"pageSize":[NSString stringWithFormat:@"%d",pagesize]} requestSerializerType:RequestSerializerTypeJson successBlock:^(RequestModel *respondObject) {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[JHBuyerCustomizeOrderListCell class] forCellWithReuseIdentifier:NSStringFromClass([JHBuyerCustomizeOrderListCell class])];
        [_collectionView registerClass:[JHSellerCustomizeOrderListCell class] forCellWithReuseIdentifier:NSStringFromClass([JHSellerCustomizeOrderListCell class])];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section==0 ) {
        ...
        if (self.isSeller) {
            JHSellerCustomizeOrderListCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([JHSellerCustomizeOrderListCell class]) forIndexPath:indexPath];
            cell.delegate=self;
            [cell setOrderMode:self.orderModes[indexPath.row]];
            
            return cell;
        }
        JHBuyerCustomizeOrderListCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([JHBuyerCustomizeOrderListCell class]) forIndexPath:indexPath];
        cell.delegate=self;
        [cell setOrderMode:self.orderModes[indexPath.row]];
