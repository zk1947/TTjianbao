...
@interface JHCustomizeBuyerOrderView ()
{
    BYTimer *timer;
}
...
@end
@implementation JHCustomizeBuyerOrderView
...
-(void)showReportTipImage{
    ...
    if (!self.orderMode.customizeShowBtnVo.reportReadFlag ) {
        for (id obj in self.buttonBackView.subviews) {
            if ([obj isKindOfClass:[UIButton class]]) {
                UIButton * button =(UIButton*)obj;
                //JHOrderButtonTypeDetail
                if (button.tag == JHCustomizeOrderButtonAppraiseReport) {
