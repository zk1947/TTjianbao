//
//  JHCustomizeCheckStuffDetailPictsTableViewCell.m
//  TTjianbao
//
...
@interface JHCustomizeCheckStuffCollectionViewCell : UICollectionViewCell
@property (nonatomic, strong) UIImageView *iconImageView;
@property (nonatomic, strong) UIView      *videoCoverView;
@property (nonatomic,   weak) UIImageView *videoIconImageView;

@property (nonatomic, strong) JHCustomizeCheckStuffDetailPictsModel *model;
- (void)setViewModel:(id)viewModel;
@end

@implementation JHCustomizeCheckStuffCollectionViewCell
...
- (void)setViewModel:(id)viewModel {
    JHCustomizeCheckStuffDetailPictsModel *model = [JHCustomizeCheckStuffDetailPictsModel cast:viewModel];
    self.model = model;
    if (model.type == 0) { /// 图片
...
@interface JHCustomizeCheckStuffDetailPictsTableViewCell ()<
...
@implementation JHCustomizeCheckStuffDetailPictsTableViewCell
...
- (void)setupViews {
    ...
    mpCollectionView.contentInset                   = UIEdgeInsetsMake(0, 10.f, 0.f, 0.f);
    [self.contentView addSubview:mpCollectionView];
    self.mpCollectionView                           = mpCollectionView;
    [mpCollectionView registerClass:[JHCustomizeCheckStuffCollectionViewCell class] forCellWithReuseIdentifier:NSStringFromClass([JHCustomizeCheckStuffCollectionViewCell class])];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    JHCustomizeCheckStuffCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([JHCustomizeCheckStuffCollectionViewCell class]) forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    [self.imgDataArray removeAllObjects];
        for (JHCustomizeCheckStuffDetailPictsModel *model in self.dataSourceArray) {
