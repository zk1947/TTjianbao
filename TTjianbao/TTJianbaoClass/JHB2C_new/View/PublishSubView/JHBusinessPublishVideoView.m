...
@interface JHBusinessPublishVideoView ()
/// 描述label
...
@implementation JHBusinessPublishVideoView

- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        self.maxDetailPictureCount = 1;
        [self setItems];
        [self layoutItems];
    }
    return self;
...
- (void)addProductVideoDetailPictureWithName:(JHRecyclePhotoInfoModel *)model{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        imageView.type = JHRecyclePhotoSeletedViewType_B2CVideo;
...
//                    model.mediaUrl = photoView.model.aliossUrl;
            ...
            [JHBrowserViewController showBrowser:@[model] currentIndex:0 from:JHRootController];
        };
...
-(void)setNetDataWithArray:(JHIssueGoodsEditImageItemModel *)modelArr{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        JHIssueGoodsEditImageItemModel *model = modelArr;
        JHRecyclePhotoSeletedView *imageView = [[JHRecyclePhotoSeletedView alloc] init];
        ...
        [imageView setDeleteBlock:^(JHRecyclePhotoSeletedView * _Nonnull photoView) {
            @strongify(self);
            [self removePictureView:photoView];
            self.publishModle.videoDetail = nil;
        }];
        [self addSubview:imageView];
        [self.imageViewArr addObject:imageView];
        [self layoutItems];
