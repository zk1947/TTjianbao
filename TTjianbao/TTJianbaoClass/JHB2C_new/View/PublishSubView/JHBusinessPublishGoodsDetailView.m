//
//  JHBusinessPublishGoodsDetailView.m
//  TTjianbao
//
//  Created by liuhai on 2021/8/12.
...
@interface JHBusinessPublishGoodsDetailView()
...
@end
@implementation JHBusinessPublishGoodsDetailView
...
- (void)addPicture:(UIButton*)sender{
    ...
    if (self.addProductDetailBlock2) {
        self.addProductDetailBlock2(self.maxDetailPictureCount - self.imageViewArr.count);
    }
}
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        for (int i = 0; i< modelArr.count; i++) {
            ...
            [imageView setDeleteBlock:^(JHRecyclePhotoSeletedView * _Nonnull photoView) {
                @strongify(self);
                [self removePictureView:photoView];
                [self.publishModle.detailImages removeObject:model];
            }];
            [self addSubview:imageView];
            [self.imageViewArr addObject:imageView];
        }
...
- (UILabel *)tipLabel{
    if (!_tipLabel) {
        ...
        desclabel.text = @"！至少添加一张商品详情图片，宽度限制750，限制5M以内";
        _tipLabel = desclabel;
    }
